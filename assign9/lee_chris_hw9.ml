...
let rec psums s = fby (head s) (fun () -> (psums (fby (head s + head (tail s)) (fun () -> tail (tail s)))))

let rec periodic l = 
  match l with 
    |[] -> fail "Function periodic given empty list"
    |hd::tl -> fby hd (fun () -> periodic (tl@[hd]))

let rec running_max s = fby (head s) (fun () -> 
        if ((head s) > (head (tail s))) then 
        running_max (fby (head s) (fun()->(tail (tail s)))) else 
        running_max (tail s))
...
let rec psumsf s = fby (head s) (fun () -> (psumsf (fby (head s +. head (tail s)) (fun () -> tail (tail s)))))


let arctan z = psumsf (map (fun n -> ((-1.0)**n)*.(z**(2.0*.n +. 1.0))/.(2.0*.n +. 1.0)) (map float nats))
...
let derivative f x = map (fun n -> ((f (x +. 1.0/.n)) -. (f x))/.(1.0/.n)) (map (fun n -> float_of_int n) (tail nats))

let rec limit mx eps s = 
  if mx = 1 then None
  else if abs_float ((head s) -. (head (tail s))) < eps
    then Some (head (tail s)) else 
...
(* CRAP I DIDN'T SEE THE HINTS *)
let rec getListNats n n' s = 
  if n = 0 then [(nth n (nth (n'-n) s))] 
  else [(nth n (nth (n'-n) s))] @(getListNats (n-1) n' s)

let stripes s = map (fun n -> getListNats n n s) nats


let rec flatten s = 
  match (head s) with 
    |[] -> flatten (tail s)
    |hd::tl -> fby hd (fun () -> flatten (fby tl (fun() -> tail s)))
