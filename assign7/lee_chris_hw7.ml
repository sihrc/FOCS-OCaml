...
let same_pair () = 
	let delta (p,a) = 
		...
		| ("start", None), sym when (isDigit sym) -> (("prehash", None),sym,Right)
		| ("start", None), sym -> (("rej", None), sym, Right)

		| ("prehash", None), "#" -> (("posthash", None), "#", Right)
		| ("prehash", None), sym when (isDigit sym) -> (("prehash", None), sym, Right)
		| ("prehash", None), sym -> (("rej", None), sym, Right)

		| ("posthash", None), sym when (isDigit sym) -> (("posthash", None), sym, Right)
		| ("posthash", None), "_" -> (("moveL", None), "_", Left)
		| ("posthash", None), sym -> (("rej", None), sym, Right)

		| ("moveL", None), ">" -> (("scanR", None), ">", Right)
		| ("moveL", None), sym -> (("moveL", None), sym, Left)

		| ("scanR", None), "#" -> (("finish", None), "#", Right)
		| ("scanR", None), sym when (isDigit sym) -> (("toHash", Some sym), "X", Right)
		| ("scanR", None), sym -> (("scanR", None), sym, Right)

		| ("toHash", Some t), "#" -> (("find", Some t), "#", Right)
		| ("toHash", Some t), sym -> (("toHash", Some t), sym, Right) 

		| ("find", Some t), sym when t = sym -> (("moveL", None), "X", Left)
		| ("find", Some t), "X" -> (("find", Some t), "X", Right)
		| ("find", Some t), sym -> (("rej", None), sym, Right)

		| ("finish", None), "_" -> (("acc", None), "_", Right)
		| ("finish", None), "X" -> (("finish",None), "X", Right)
		| ("finish", None), sym -> (("rej", None), sym, Right)

		| ("acc", None), sym -> (("acc",None), sym, Right)
		| ("rej", None), sym -> (("rej",None), sym, Right)
		| _, sym -> (("rej", None), sym, Right)
	in
	TM.build string_of_state_sym_opt { states = (crossSome ["toHash"; "find"] digits)@(crossNone ["start";"moveL";"scanR";"finish";"rej";"acc";"prehash";"posthash"]);
		input_alph = "#"::digits;
		tape_alph = ["#";"X";"_";">"] @ digits;
		leftmost = ">";
		blank = "_";
		delta;
		start = ("start", None);
...
let pred_pair () = 
	let delta (p,a) = 
		...
		| ("posthash", None), sym when (isDigit sym) -> (("posthash", None), sym, Right)
		| ("posthash", None), "_" -> (("moveL", Some "0"), "_", Left)
		| ("posthash", None), sym -> (("rej", None), sym, Right)

		| ("moveL", Some d), "#" -> (("findu", Some d), "#", Left)
		| ("moveL", Some d), ">" -> (("rej", None), ">", Right)
		| ("moveL", Some d), sym -> (("moveL", Some d), sym, Left)

		| ("findu", Some d), "X" -> (("findu", Some d), "X", Left)
		| ("findu", Some d), ">" -> (("finish", None), ">", Right)
		| ("findu", Some d), sym when (isDigit sym) && d = "0" -> if sym = "0" then (("moveR", Some "9"), "X", Right) else (("moveR", Some (string_of_int ((int_of_string sym) - 1))), "X", Right) 
		| ("findu", Some d), sym when (isDigit sym) -> (("moveR", Some sym), "X", Right) 

		| ("moveR", Some t), "_" -> (("findv", Some t), "_", Left)
		| ("moveR", Some t), sym -> (("moveR", Some t), sym, Right)

		| ("findv", Some t), "X" -> (("findv", Some t), "X", Left)
		| ("findv", Some t), sym when t = sym && not (t = "9") -> (("moveL", Some "1"), "X", Left)
		| ("findv", Some t), sym when t = sym -> (("moveL", Some "0"), "X", Left)
		| ("findv", Some t), sym -> (("rej", None), sym, Right)

		| ("finish", None), "_" -> (("acc", None), "_", Right)
		| ("finish", None), "X" -> (("finish",None), "X", Right)
		| ("finish", None), "#" -> (("finish",None), "X", Right)
		| ("finish", None), sym -> (("rej", None), sym, Right)
		...
		| _, sym -> (("rej", None), sym, Right)
	in
	TM.build string_of_state_sym_opt { states = (crossSome ["findu";"moveL"] ["0";"1"])@(crossSome ["moveR";"findv"] digits)@(crossNone ["start";"prehash";"posthash"; "finish"; "acc"; "rej"]);
		input_alph = "#"::digits;
		tape_alph = ["#";"X";"_";">"] @ digits;
		leftmost = ">";
		blank = "_";
...
let transf_states m = 
	List.filter (fun x -> match x with (a,Some b) -> not (a=b) |_ -> true) (List.fold_right (fun x y -> (x, None)::(List.map(fun z -> (x,Some z)) m.states_S)@y) m.states_S []);;

let transf_delta m (st,sym) = 
	let delta = m.delta_S in
	match st with 
	| (newSt,None) -> 
		(match delta (newSt,sym) with 
		| (state,write,move) when move = S -> ((newSt, Some state), write, Right)
		| (state, write, move) when move = R -> ((state, None), write, Right)
		| (state, write, move) -> ((state, None), write, Left))
	| (newSt, Some b) when not(sym = m.leftmost_S) -> ((b, None), sym, Left)
	| (newSt, Some b) -> ((m.reject_S,None),sym,Right)
